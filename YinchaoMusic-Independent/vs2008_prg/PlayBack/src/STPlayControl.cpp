...
STBool STPlayControl::IsAllJobToDone()
{
	if (iCurPlugin != NULL)
	{
		...
		while(pBuffer != NULL)
		{
			iSampleBufferManager->RecycleFreeBuffer(pBuffer);//回收buffer,好供其它需要的地方再使用
		...
		if (ESTReadStatusReading == tReadStatus)
		{
			...
			if (pBuffer != NULL)
			{			
				STInt nErr = iCurPlugin->Read(pBuffer);//从文件读出并解码后放入pBuffer
...
void STPlayControl::Start()
{
	...
	{
		if (STKErrNone == FillAudioOutput())//开始读文件
		{
			iAudioOutput->Start();//开始播放
...
STInt STPlayControl::FillAudioOutput()
{
	...
	for (STInt i = 0; i < tAudioInfoArray.Count(); i++)
	{
		...
		if (STKErrNone == (nErr = iCurPlugin->Read(pBuffer)))//读音频文件内容放入buffer
		{
			iAudioOutput->RenderBuffer(pBuffer);
		}
		else
