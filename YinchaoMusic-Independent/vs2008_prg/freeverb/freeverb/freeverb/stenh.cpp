				 ...
				 long numsamples)
{
  for(long i = 0;i < numsamples;i ++)
    {
      fv3_float_t iL = inputL[i], iR = inputR[i],
	diff, directS, delayS, sumS, gainS, gainD, vcaFactor;
      UNDENORMAL(iL); UNDENORMAL(iR);
...
void FV3_(stenh)::setThreshold(fv3_float_t dB){ thres_dB = dB; update(); }
...
void FV3_(stenh)::setRatio(fv3_float_t value){ Ratio = value; update(); }

void FV3_(stenh)::setSoftKnee(fv3_float_t dB){ SoftKnee = dB; update(); }
