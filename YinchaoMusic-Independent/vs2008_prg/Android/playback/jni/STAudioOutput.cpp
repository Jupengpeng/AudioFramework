...
STInt STAudioOutput::Open(STInt aSampleRate, STInt aChannels, const STChar* aRecordSavePath)
{
	...
	if (aRecordSavePath != NULL && strlen(aRecordSavePath) != 0)
	{
		...
		iDenoise->init(aSampleRate,1024);//暂时写死

		iAudioRecorder = new STAudioRecorder(*iOpenSLESEngine, *this);
...
{
	STBaseAudioOutput::RenderBuffer(aBuffer);//把要播放的数据先放入播放队列

	if (IsUnderflow())//底层播放数据已用完
	{
		FillBuffer();//从播放队列取出一个buffer放入底层播放队列播放（真正的render动作）
	}
}
...
STSampleBuffer*	STAudioOutput::GetFilledBuffer()
{
    ...
    if (iCurSampleBuffer != NULL)
    {
		...
		//获得新buffer之前，把当前buffer放入iEmptiedBackgroundBufferArray，录制合成要用
...
//modify bin.liu 加入buffer回收阶段加入降噪处理，注buffer回收时，首先判断buffer是否大于2048*2个字节大小，如果小于就放弃回收，等待下次回调
...
// 				if(nProcessSize < 2048*2/*一个降噪处理单位需要的内存块大小*/)
// 				{
// 					//返回null ，表示没有回收
// 					return NULL;
// 				}
//				iDenoise->Process(iRecordLoopBufferStartPtr,nProcessSize,NULL/*转入null表示，iRecordLoopBufferStartPtr 作为降噪处理返回值*/);
...
//			STLOGE("iRecordLoopBufferValidSize2: %d", iRecordLoopBufferValidSize);
	...
	return STBaseAudioOutput::RecycleBuffer(aForceRecycle);//真正从缓存队列中删除解码后的数据（如果删除就没有机会播放了）
}
...
//真正的往底层扔数据（播放动作）
void STAudioOutput::FillBuffer()
{
	iCritical.Lock();
	if (iCurSampleBuffer == NULL || iCurSampleBuffer->ValidSize() == 0)//如果当前buffer还有数据末送入底层播放队列，则不用从新获得填充buffer。
	{
...
//	}
	...
	return iAacEncoder->Execute(iMixBuffer, iProcessedByteOffset - size);//aac编码，可以做成其它编码形式
}
...
//		STLOGE("recordPos:%d nPlayPos:%d, recordDelay: %f", nRecordPos, nPlayPos, (float)(nRecordPos - nPlayPos)/iSampleRate/2/iChannels);
	...
	{
		STUint8* pEndPtr = iRecordLoopBufferStartPtr + iRecordLoopBufferValidSize;//获得循环内存有效数据的结尾指针（准备向循环内存里面考贝新到来的数据）
		if (pEndPtr >= iRecrodLoopBuffer + KRecordLoopBufferSize)//指针大于最大循环内存的结尾指针时，返转到
		{
		...
		{
			STInt nEndRemainSize = iRecrodLoopBuffer + KRecordLoopBufferSize - pEndPtr; //循环数组最大地址最后还剩余多少
