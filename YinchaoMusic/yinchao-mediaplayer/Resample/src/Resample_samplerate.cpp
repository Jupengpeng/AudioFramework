...
const char *
src_get_name_st (int converter_type)
{	const char *desc ;
	...
	if ((desc = linear_get_name_st (converter_type)) != NULL)
		return desc ;
...
const char *
src_get_description_st (int converter_type)
{	const char *desc ;
	...
	if ((desc = linear_get_description_st (converter_type)) != NULL)
		return desc ;
...
int
src_is_valid_ratio_st (double ratio)
{
...
void
src_short_to_float_array_st (const short *in, float *out, int len)
{
	while (len)
	{	len -- ;
...
void
src_float_to_short_array_st (const float *in, short *out, int len)
{	double scaled_value ;
...
psrc_set_converter (SRC_PRIVATE	*psrc, int converter_type)
{
	...
	if (linear_set_converter_st (psrc, converter_type) == SRC_ERR_NO_ERROR)
		return SRC_ERR_NO_ERROR ;
